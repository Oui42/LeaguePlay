<?php
$list = array();
$sql = "SELECT u.*, t.* FROM `lp_tickets` AS `t` LEFT JOIN `lp_users` AS `u` ON u.uid = t.author WHERE t.status = ".$__ticketStatus['active']." OR (t.admin > '0' AND t.status != '".$__ticketStatus['closed']."') ORDER BY t.ticket_id DESC";
$query = query($sql);
...
<table class="table">
	<tr class="table-tr">
		...
		<th class="table-th" width="150">Admin</th>
